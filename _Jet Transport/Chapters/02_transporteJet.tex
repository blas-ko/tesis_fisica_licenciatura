...
\begin{equation}
 H(p,\theta) = K + U = \frac{p_{\theta}}{2ml^2} + mgl\left(1 - \cos{\theta} \right).
\label{eq:pendulo-ham}
\end{equation}
 
Las EDO quedan entonces, por \ref{eq:ham-rel}, como
\begin{align}
 \dot{\theta} &= \frac{p_{\theta}^2}{ml^2} \nonumber \\
...
Si tomamos $l=1 m, m=1 kg, g = 10 \frac{m}{s^2}$ entonces, para las condiciones iniciales $p_{\theta}(0) = 2 N \cdot m$ y $\theta(0) = \pi/3$, la energía total es 
\begin{equation*}
E_0 = H(p_{\theta}(0),\theta (0) ) = 7 J$
\end{equation*}

En la figura \ref{fig:pendulo-energy} se puede ver las variaciones de energía $E(t) - E_0$ en función del tiempo.
