...
En la figura \ref{fig:FIGURA!}, se grafica la diferencia entre las soluciones real para distintos radios alrededor de $\xo$ y los calculados por el TJ\footnote{Las soluciones para (\ref{eq:center}) se obtuvieron con el método de Euler, para $h=10^{-4}$.}
...
\begin{equation}
 ||P(\mathbf{\xi})||_p := \left( \sum_{k} ||a_k||_p \right)^{1/p}
 \label{eq:poly-norm}
\end{equation}  
