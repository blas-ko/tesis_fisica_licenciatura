...
Sea $\dot{\mathbf{x}}(t) = \mathbf{f}(\mathbf{x}(t),t) = A\mathbf{x}+\mathcal{O}(\mathbf{x}^2)$ un sistema de $d$ grados de libertad que describe un campo vectorial con parte lineal $A \mathbf{x}$. Si nos quedamos con la parte lineal de $f$, podemos encontrar la solución explícita al sistema
\begin{equation*}
...
Dada ésta solución, vemos que soluciones vecinas son simplemente 
\begin{equation*}
 (\mathbf{y} + \mathbf{\delta y})(t) = (\mathbf{y}_0 + \mathbf{\delta y}_0) e^{\Lambda t}
\end{equation*}

y luego regresar al sistema de coordenadas original. Para esto, buscamos eigenvectores tales que $\det(P) = 1$, de modo que la transformación preserve área y orientación.
...
\begin{equation}
 ...
 \label{eq:lyap-sep}
\end{equation}
dada la solución a un campo vectorial con parte lineal $A$. $\lambda$ aquí es el máximo eigenvalor de todos los $\lambda_i$'s. Necesariamente, la variación $\delta x$ debe ser suficientemente pequeña para que $A\mathbf{x}$ sea el término dominante de $f$ aunque, en el límite $\delta x \to 0$, esto es cierto por completo. 

Los ELTF son, en escencia, indicadores escalares que, para cada punto $\mathbf{x}$ del espacio fase, nos dice qué tan grande será la tasa de separación con soluciones cercanas. Estos se obtienen de despejar \ref{eq:lyap-sep}
\begin{equation}
...
donde $J^t(\xo) = \frac{\partial \phi(t;t_0,\xo)}{\partial \mathbf{x}_{0_j}}$.
 
Con esto, obtenemos una forma más práctica de obtener los ELTF
\begin{equation}
...
%choro: Intro de algunos de los métodos para detectar LCS y separatrices sin usar JT, hablar del FTLE y del PS (Poincaré Sections) y así ... 
