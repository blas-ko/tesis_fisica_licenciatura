...
\begin{figure}[h!]
 ...
 \caption{Transporte de jets para la ecuación \ref{eq:center} con condición inicial $\xo = (0,0)^T$ usando el método de Euler para $\Uxo$ parametrizada en un círculo $\xi(\tau) = r\left( \cos(\tau),\sin(\tau) \right)$ en un intervalo temporal $[0,2\pi]$ en pasos de $h=10^{-4}$. Se evaluó la solución para los distintos $r$'s marcados en la gráfica, en intervalos de $1000h$.}
...
En los ejemplos anteriores se ha evaluado la precisión del método de Taylor con y sin el transporte de jets. Hemos estudiado casos hamiltonianos en donde las soluciones a las ecuaciones diferenciales viven en curvas de energía constante sobre el espacio fase. Es importante tener en cuenta que éste es sólo un subconjunto de casos en la familia de sistemas de ecuaciones diferenciales ordinarias como órbitas periódicas, atractores o atractores extraños \cite{Perez2015}. Nuevos elementos aparecen en los campos vectoriales impuestos por la forma más general de (\ref{eq:ode}). La idea de los indicadores dinámicos radica en encontrar estructuras que nos permitan catalogar a los campos vectoriales, sean hamiltonianos o no, y sacar información relevante acerca de las soluciones al sistema de ecuaciones dado. 

\subsection{Un poco de motivación vía exponentes de Lyapunov}

El transporte de jets es relevante en el sentido de poder hacer variaciones de tamaño $\delta x$ alrededor de $\xo$ y ver cómo evoluciona el flujo $\flowU$ de toda la vecindad en el tiempo. Respecto a esto, son conocidos los esfuerzos realizados en saber qué pasa en la vecindad de una condición inicial bajo la aproximación lineal del campo vectorial que define la trayectoria. De aquí, surge la idea de los \textit{Exponentes de Lyapunov a Tiempo Finito}, o ELTF, que busca encontrar en el espacio fase componentes de caos \cite{Perez2015} o estructuras lagrangianas y separatrices \cite{Perez2015}. 
...
y luego regresar al sistema de coordenadas original bajo $P$. Para esto, buscamos eigenvectores tales que $\det(P) = 1$, de modo que la transformación preserve área y orientación.

Con lo anterior podemos plantear una forma intuitiva en la que se separan dos soluciones cercanas
\begin{equation}
 || \mathbf{\delta x}(t) || = e^{\lambda t} || \mathbf{\delta x}_0 ||.
 \label{eq:lyap-sep}
\end{equation}
$\lambda$ es el máximo eigenvalor de todos los $\lambda_i$'s de $\Lambda$. Notemos que la variación $\delta x$ debe ser suficientemente pequeña para que $A\mathbf{x}$ sea el término dominante de $f$ aunque, en el límite $\delta x \to 0$, esto es cierto por completo. 
...
\begin{equation}
 \lambda_{\xo}^t = \frac{1}{t}\ln \left( \frac{||\mathbf{ \delta x(t)} ||_2}{|| \mathbf{\delta x}_0 ||_2} \right)
 \label{eq:lyap-exp}
\end{equation}
...
\begin{equation*}
 \delta \mathbf{x}(t) = J^t(\xo) \cdot \delta \xo
\end{equation*}

con 
\begin{equation}
 J^t(\xo) = \frac{\partial \phi(t;t_0,\xo)}{\partial \mathbf{x}_{0_j}}
 \label{eq:jacobian}
\end{equation}
la matriz jacobiana para $\flowci$ alrededor de $\xo$. Con esto, obtenemos una forma más práctica de obtener los ELTF
\begin{equation}
 \lambda_{\xo}^t = \frac{1}{t}\ln \left( ||J^t(\xo) ||_2 \right)
\end{equation}
...
Geométricamente, (\ref{eq:jac_jt}) representa la deformación lineal de una variación $\delta \mathbf{x}$ respecto de $\xo$ para cada paso en $t$. Notemos que para $t = t_0$, $J^0(\xi) = \mathbf{1}$.
...
Intuitivamente, y suponiendo que la expansión en series de $\flowxi$ corresponde a una expansión analítica, los coeficientes de la expansión serán menores entre mayor sea el orden. Dicho esto, una forma de controlar qué tan grandes pueden ser las variaciones en las vecindades alrededor de $\xo$ es acotar la contribución del último término y pedir que
\begin{equation*}
 a_{m,k}\xi_{max}^k \leq \epsilon_{jet}
\end{equation*}
%Ver notación y redacción de este último párrafo.
