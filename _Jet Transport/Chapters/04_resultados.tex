...
Para reforzar este análisis, se computaron las direcciones de mayor expansión para la vecindad de los puntos lagrangianos. La figura \ref{fig:C3BP_seprate3} presenta dichas direcciones en distintas secciones del espacio de configuraciones, donde se observa cómo en las zonas de menor expansión, equivalentes a las zonas donde $\xi_{max}$ es pequeña,  $\theta_{+}$ cambia drásticamente. Esto nos hace entender a las curvas como una especie de separatriz del problema. La figura presenta un acercamiento para cada punto lagrangiano (exeptuando a $L_5$ que es equivalente a $L_4$). Para $L_3$ y $L_2$, que están a la izquierda de la masa primaria mayor y a la derecha de la menor, respectivamente, se observa como las vecindades se expanden hacia la singularidad hasta que, cuando cruzan $\mathcal{C}_s$, buscan escapar de ella; la misma dinámica ocurre para $L_4$. En el caso de $L_1$, por donde cruzan varias curvas $\mathcal{C}_s$ la dirección $\theta_{+}$ puede regresar a lo que tendía inicialmente si el número de cruces es par y esto dará la tendencia de cuáles condiciones van hacia la masa primaria mayor y cuáles hacia la menor. 

%FIGURE!
\begin{figure}
 \centering
 \includegraphics[width=0.8\linewidth]{C3BP_seprate3}
 \caption{Espacio de configuraciones para $\phi_\mu$ con variaciones $\delta\mu = 0$ (azul), $\delta\mu = \delta\mu_{max}$ (naranja), y $\delta\mu = -\delta\mu_{max}$ (verde), donde $\delta\mu_{max} = \xi_{max}(\phi_\mu) \approx 0.00329$. Para el transporte se usó un jet de orden $n=18$ con condiciones iniciales $\left( L_{4_x}, L_{4_y} + \Delta y, 0, 0, \mu \right)$.}
