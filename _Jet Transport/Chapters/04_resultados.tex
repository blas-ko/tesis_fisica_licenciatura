...
El análisis de eigenvalores alrededor de los puntos singulares nos da cómo es la estabilidad de dichos puntos, sin embargo, también es interesante ver las soluciones cerca de $\mu_c$ ya que nos permitirá ver directamente en el flujo el comportamiento de estabilidad de la partícula que visita a $L_4$ o $L_5$.
...
\begin{equation}
 ...
 \label{eq:hp_matrix}
\end{equation}  
al derivar (\ref{eq:hp-filter}) por $\tau_t$, donde
\begin{align}
...
ya que ésta es fácilmente programable. Así, de (\ref{eq:hp_matrix}), obtenemos que $\mathbf{\tau}_t = \left( \lambda \mathbf{F} + \mathbf{I}_t \right)^{-1} y_t$ y $c_t = y_t - \tau_t$. Separando de esta manera a la figura \ref{fig:remoteness_dmu}, podemos obtener el ciclo y la tendencia tal como se presenta en la figura \ref{fig:trendcycle_dmu}. En ésta, es bastante claro cómo desde la primera variación negativa ($\delta\mu = -\delta\mu_{max}/3$) para abajo, la tendencia decrece después de determinado tiempo. Esto quiere decir que la distancia respecto al punto inicial no diverge, como se había predicho en el análisis de estabilidad lineal de (\ref{eq:stability_CR3BP}). Por otro lado, es interesante ver cómo la amplitud de las oscilaciones en la parte cíclico de $\Delta\phi_\mu$ son mayores entre mayor sea $\delta_\mu$.
...
Se construyeron en la sección \ref{sec:3BP} los potenciales (\ref{eq:3body_potential}) y sus respectivas energías cinéticas (\ref{eq:3body_kinetic}) del problema general de tres cuerpos. Estas nos permiten definir al hamiltoniano
\begin{equation}
...
\end{equation}
es el hamiltoniano para cada una de los cuerpos. Notemos que (\ref{eq:3BP_hamiltonian}) no depende del tiempo y, por lo tanto, es un sistema conservativo. 

Cuando en la sección \ref{sec:R3BP} se supuso que la masa menor no inlfuye en la dinámica de las masas primarias, bastó con hacer una rotación con velocidad angular $\Omega$ para encontrar las ecuaciones de movimiento de $m_3$. Esto fija las masas primarias y modifica el hamiltoniano para la masa menor ya que el potencial de $M_2$ se cancela con el de $M_1$. Dicha rotación se puede realizar bajo el cambio de coordenadas
\begin{align*}
 x &= X\cos (\Omega t) - Y \sin (\Omega t) \\
 y &= X\sin (\Omega t) - Y \cos (\Omega t) 
\end{align*} 
con $(x,y)$ y $(X,Y)$ las nuevas y viejas coordenadas para $m_3$, respectivamente, tal como se muestra en la figura \ref{fig:}.

Para obtener el hamiltoniano modificado, que en alguna literatura se llama Kamiltoniano \cite{Goldstein2007, etc}, es necesario ver cómo se modifica momento conjugado en el nuevo sistema y, para esto, se debe conservar el principio de mínima acción

\begin{align}
 \delta \int_{t_0}^t \left( \dot{\mathbf{r}} \cdot \mathbf{p} - K(\mathbf{r}, \mathbf{p}, \tau) \right) d\tau &= 0 \nonumber \\
...
\end{equation}
donde $\mathbf{P} = (P_x,P_y)$, $\mathbf{R} = (X,Y)$, $\mathbf{p} = (p_x,p_y)$, $\mathbf{r} = (x,y)$, $K$ es el Kamiltoniano y $G$ es una función que genera una transformación canónica entre ambos sistemas, también conocida como función generatriz de tipo 2 \cite{generatriz}. Notemos que como $\frac{dG}{dt} = \pder{G}{\mathbf{R}}\dot{\mathbf{R}} + \pder{G}{\mathbf{p}}\dot{\mathbf{p}} + \pder{G}{t}$ entonces
\begin{align}
 \mathbf{P} &= \pder{G}{\mathbf{R}} \nonumber \\
 \mathbf{r} &= \pder{G}{\mathbf{p}} \nonumber \\
 \therefore K(\mathbf{r}, \mathbf{p}) &= H(\mathbf{r}, \mathbf{p}) + \pder{G}{t} (\mathbf{r}, \mathbf{p}).
 \label{eq:kamiltonian_formulae}
\end{align}

$G$ se define simplemente como
\begin{equation}
 G(\mathbf{R}, \mathbf{p}, t) = p_x \underbrace{\left( X \cos (\Omega t) - Y \sin (\Omega t) \right) }_{x} + p_x (\underbrace{ \left( X \sin (\Omega t) + Y \cos (\Omega t) \right)}_{y} .
 \label{eq:generating_function}
\end{equation}

Con esto y las ecuaciones de (\ref{eq:kamiltonian_formulae}) podemos escribir explícitamente el kamiltoniano como
\begin{equation}
...
\end{equation}
\begin{equation}
 \therefore K = \frac{1}{2M_3} \left( (p_x - \Omega y)^2 + (p_y + \Omega x)^2 \right) - \left(G \frac{M_1}{r_{1,3}} + G \frac{M_2}{r_{2,3}} + \frac{\Omega}{2}\left( x^2 + y^2 \right) \right).
 \label{eq:kamiltonian}
\end{equation}

%FIGURA!

Así, para probar la simplecticidad del PC3C, basta con hacer las transformaciones 
\begin{align*}
p_x &\to p_x - \Omega y \\
p_y &\to p_y + \Omega x
\end{align*} 
y aplicar entonces la relación (\ref{eq:sympletic_flow}).

No se puede conseguir simplecticidad en $h_3$ sin hacer la transformación canónica dado que sin ésta, la presencia de las dos masas primarias sí influyen en el potencial. En la figura \ref{fig:} se muestra el ingenuo intento de esto. Al hacer la rotación angular $\Omega t$ aparecen la fuerza centrípeta y la fuerza de corielos, que se ven presentes en (\ref{eq:kamiltonian}). En partícular, la fuerza de Coriolis nos lleva a (\ref{eq:coriolis_transformations}), la cual es necesaria para la conservación simpléctica. La figura \ref{fig:}. 

%Hablar de cómo el jet trasnport ayuda para todo esto. 
%Usar el teorema del hamiltoniano trunco
%Meter gráficas