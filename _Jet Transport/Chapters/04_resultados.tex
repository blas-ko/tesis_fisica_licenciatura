...
ya que ésta es fácilmente programable. Así, de (\ref{eq:hp_matrix}), obtenemos que $\mathbf{\tau}_t = \left( \lambda \mathbf{F} + \mathbf{I}_t \right)^{-1} y_t$ y $c_t = y_t - \tau_t$, lo cual aplicamos a la figura \ref{fig:remoteness_dmu}, lo cual se refleja en la figura \ref{fig:trendcycle_dmu}. En ésta, es bastante claro cómo desde la primera variación negativa ($\delta\mu = -\delta\mu_{max}/3$) para abajo, la tendencia decrece después de determinado tiempo. Esto quiere decir que la distancia respecto al punto inicial no diverge, como se había predicho en el análisis de estabilidad lineal de (\ref{eq:}). Por otro lado, es interesante ver cómo la amplitud de las oscilaciones en la parte cíclico de $\Delta\phi_\mu$ son mayores entre mayor sea $\delta_\mu$.
