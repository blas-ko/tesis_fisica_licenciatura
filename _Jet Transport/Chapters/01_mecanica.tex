...
Antes que nada, supondremos que la tercer partícula será mucho más pequeña que las otras dos, i.e., $m_3 \ll M_2 \leq M_1$. Así, una primera aproximación es que $m_3$ no afecta la dinámica de $M_2$ ni de $M_1$ y, por tanto, la solución para $U_{M_1}$ y $U_{M_2}$ es la de los dos cuerpos desarrollada en \ref{sec:2body_problem}, es decir, (\ref{eq:2body_solution}). Sólo resta resolver cómo se mueve $m_3$, la cual sí es afectada por los dos cuerpos masivos.
...
Para trabajar la dinámica de $m_3$ conviene establecer un marco de referencia en rotación de modo que $M_1$ y $M_2$ estén en reposo. Dicha rotación tendrá velocidad angular $\Omega$ y estará basada en el centro de gravedad entre las dos. Recordemos, de la sección \ref{sec:ficticious_forces}, que un sistema de referencia en rotación traerá algunas fuerzas ficticias descritas por (\ref{eq:rotating_constant_acceleration}), más específicamente la aceleración de Coriolis y la aceleración centrípeta.
...
\end{equation} 
la parte cerrada del potencial, que se suele conocer como \textit{pseudo-potencial}, ya que si la velocidad es distinta de cero, la fuerza de Coriolis debe ser considerada para la dinámica de la partícula. Notemos que las coordenadas en (\ref{eq:3body_restricted_acceleration}) van de acuerdo a la figura \ref{fig:3body_scheme}, donde $M_1$ y $M_2$ están sobre el eje $x$.

%FIGURA!

Aún si no describe completamente a $m_3$, estudiar la estructura del pseudo-potencial nos da una buena intuición sobre los puntos de estabilidad y equilibrio. Éste análisis nos ayudará a entender mejor la estructura en el espacio de configuración $xy$ y encaminará a los indicadores que se desarrollan en el siguiente capítulo.

La figura \ref{fig:3body_pseudo-potential} muestra la proyección de $V$ en el plano $xy$. Cualitativamente, se pueden observar tres puntos silla sobre el eje $x$, mientras que en medio de los planetas, con alguna altura $y \neq 0$, se observan islas que parecen centros, al menos topológicamente.

%FIGURA! 

Estos puntos singulares se conocen como \textit{puntos lagrangianos} y podemos encontrarlos cuando la fuerza $\nabla V = 0$, donde
\begin{equation}
 \nabla V = PENDIENTE.
\end{equation} 
La solución para éstos involucra hallar raíces para polinomios de quinto grado, y encontrarlos de manera analítica es difícil. 

En una primera aproximación, y considerando que tres de los puntos se encuentran el la recta $y=0$, obtenemos que
\begin{align*}
 L_1 =
 L_2 =
 L_3 =
...
%Desarrollar lo mismo para los de y... 

Un análisis más profundo se hará en el capítulo XXX.