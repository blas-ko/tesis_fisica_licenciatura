...
Visto eso, es práctico definir otras operaciones que serán de ayuda al trabajar con elementos de $\pk$. Serán la derivada, la potencia y la composición  nuestros " caballos de batalla " para el desarrollo de casi cualquier función existente en la aritmética usual.

Sea $A = A(x) = \polk{a_k} \in \pk$. Inspirados en la derivada usual de polinomios de orden $n$ $A: \mathbb{C} \to \mathbb{C}$, vemos que

\begin{align*}
 \frac{dA(x)}{dx} := A'(x) = A' = \sum_{k=0}^n{k a_k x^{k-1}} = \polk{(k+1)a_{k+1}} 
\end{align*}

así, se define a la \textbf{derivada} de $A$ como
...
con $a_{n+1} := 0 \in \mathbb{K}$ tal que $A' \in \pk$.

Ahora, si $m \in \mathbb{N}$,
\begin{align*}
...
%		- Show that the "chain rule" works fine.
%		- Define (/,-,sin,cos,exp,log,pow)
