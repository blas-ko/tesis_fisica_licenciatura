...
\begin{align}
 \dot{\bm{x}(t)} := \frac{dx_i(t)}{dt} = f_i(\textbf{x}(t)) \ \forall i \in [1,n]
 \label{eq: eqdif}
\end{align}
...
\begin{align*}
 B_{0,\bm{x}_0}(\bm{\mu}) = \bm{x}_0 + \bm{\mu}
\end{align*}
...
\begin{align*}
 \bm{x_1} =&  B_{1,\bm{x}_0}(\bm{\mu}) =  B_{0,\bm{x}_0}(\bm{\mu}) + h\cdot\bm{f}( B_{0,\bm{x}_0}(\bm{\mu})) \\ 
...
%	- Esto último va mejor... Ya es claro que con bolas iniciales Euler funciona, ahora sólo queda generalizar el orden de Euler para que se vuelva un método de Taylor, discutir la elección del step-size óptimo y ver cómo se trata esta elección cuando los coeficientes son polinomios.
